...
//     programs:  [ { id, name, rounds, blocks: { warmup:[], main:[], cooldown:[], roundRest:[] } } ],
//                // roundRest opcionális: gyakorlatok (pl. nyújtás) a fő körök
//                // közötti pihenő kitöltésére, a warmup/cooldown blokkokkal
//                // azonos elem-alakban.
...
// A program blokkjai közül ez az egy nem gyakorlat-kategória: a "roundRest"
// a fő körök közötti pihenőt gyakorlatokkal (pl. könnyű nyújtásokkal) tölti
// ki a puszta visszaszámlálás helyett/mellett. Opcionális - ha üres vagy
// hiányzik, index.html a régi, egyetlen ROUND_REST_SEC hosszú pihenőre esik
// vissza.
const PROGRAM_BLOCK_KEYS = [...BLOCK_KEYS, 'roundRest'];
...
export function validate(data, opts = {}) {
  ...
  for (const [id, ex] of Object.entries(exercises)) {
    ...
    // warmup/cooldown elemeknél az időzítő nem a "main" blokk WORK_SEC
    // alapértékére esik vissza (lásd index.html buildQueue) - itt tényleg
    // kötelező a saját duration_sec, különben a hátralévő idő NaN lesz.
    if ((ex.category === 'warmup' || ex.category === 'cooldown')
      && (typeof ex.duration_sec !== 'number' || !(ex.duration_sec > 0))) {
      errors.push(`${where}: hiányzik vagy érvénytelen a "duration_sec" (kötelező, pozitív szám warmup/cooldown gyakorlatnál)`);
    }
  ...
  for (const p of data.programs || []) {
    ...
    for (const bk of PROGRAM_BLOCK_KEYS) {
